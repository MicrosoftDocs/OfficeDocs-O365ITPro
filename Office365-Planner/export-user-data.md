...
|Plan.Tasks.Recurrence|Defines active or inactive recurrence for the task. `null` when the recurrence has never been defined for the task.|
...
|Plan.Tasks.Recurrence.PreviousInSeriesTaskId|The **Task ID** of the previous task in this series. `null` for the first task in a series since it has no predecessor. Each subsequent task in the series has a value corresponding to their predecessor.|
...
|Plan.Tasks.Recurrence.Schedule.Pattern|The pattern for recurrence. The **NextOccurrenceDate** is calculated using this value, along with the **Schedule.Range**.|
...
|Plan.Tasks.Recurrence.Schedule.Pattern.Interval|The interval applied to the candence kind -- values greater than 1 mean that a period will be skipped with no occurrences. Example: for a Daily pattern, an Interval of 2 means tasks will recur every other day. For a monthly pattern, an Interval of 3 means tasks will recur every three months (aka quarterly).|
|Plan.Tasks.Recurrence.Schedule.Pattern.DaysOrDates|Each entry in this collection represents the definition of exactly one day or date. Example: "FixedYearly,August,15" means on August 15. "FloatingMonthly,Second,Monday" means on the second Monday of a month. "Weekly,Wednesday","Weekly,Friday" means weekly on Wednesdays and Fridays.|
...
|Plan.Tasks.Recurrence.Schedule.Range.EndDate|Currently unsupported; will always be null.|
|Plan.Tasks.Recurrence.Schedule.Range.NumberOfOccurrences|Currently unsupported; will always be null.|
|Plan.Tasks.Recurrence.Schedule.NextOccurrenceDate|The next date for this **Recurrence.Schedule**. When a new task is instantiated to continue the recurrence series, this date is used for the **DueDate** of the new Task. Auto-generated by the service.|
