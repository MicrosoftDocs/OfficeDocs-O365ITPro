...
1. Open https://www.qnamaker.ai/Create, and sign in with your Azure account. 
    ...
    ![Azure QnA Maker initial page](media/create-kb-step1.png)
 
2. Click **Create a QnA service**. This opens a new browser window to the Azure portal.
    ![QnA Maker Create page](media/azure-resource-group.png)
   ...
   f. After the deployment finishes, go to the Resource group.
        ![QnA Maker resource group page](media/create-qna-maker.png)
...
3. Go back to the knowledge base create page that you opened.
   ...
   ![QnA Maker step 2, Connect your QnA service to your kb](media/create-kb-step-2.png)
...
4. In the **Name** field, enter a name for the kb. For example: **Sample QnA FAQ**
 ...
 ![QnA Maker step 3, Name your kb](media/create-kb-step-3.png)
 
5. Populate the kb with a base source by clicking **+ Add file** and uploading the Sample QnA FAQ.xls file.

   ![QnA Maker step 4, Populate your kb](media/create-kb-step-4.png)
 
6. Click **Create your kb**.

   ![QnA Maker step 5, Create your kb](media/create-kb-step-5.png)
 
7. Click **Save and train**.

   ![Sample knowledge base page](media/kb-save-train.png)
 
8. Click **PUBLISH**.

   ![Sample QnA FAQ page](media/kb-publish.png)
...
From the **Success** page, copy the three strings indicated in red rectangles above. You'll use these to initialize variables in the Yammer FAQs Flow.
 ...
 ![Success message from QnA Maker](media/kb-publish-success.png)
...
1. Open an Office 365 connected Group in Yammer and click **SharePoint Site**.
   ...
   ![Sample of Office 365 connected Yammer group page](media/yammer-group-o365-resources.png)

2. Copy the site URL. You'll use this to configure the **Create item** node of the Yammer FAQs Flow.

   ![SharePoint site associated with the Office 365 connected group, showing URL for later use](media/sp-site.png)

    a. Create a new list named **QnA**. Click **+ New**, and then click **List**.

   ![SharePoint site associated with the Office 365 connected group, with New List highlighted](media/sp-new-list.png)
 
    b. In the **Name** textbox, enter **QnA**, and then click **Create**.
 
   ![New list dialog box](media/sp-new-list-dialog.png)
 
3. To rename the **Title** column to **Question**, click **Title**, click **Column settings**, and then click **Rename**.

   ![SharePoint page, showing Column settings](media/sp-qna-rename-title-column.png)
  
    a. In the **Name** textbox, enter **Question**, and then click **Save**.
 
   ![Rename column dialog box](media/sp-qna-rename-title-column-dialog.png)
 
4. To create a new column named **Answer**, click **+ Add column**, and then click **Multiple lines of text**.
 
   ![SharePoint list, showing Add column](media/sp-qna-add-column.png)
 
    a. In the **Name** textbox, enter **Answer**, and then click **Save**.
 
   ![Sample Answer column](media/sp-qna-add-column-answer.png)
...
2. Open https://flow.microsoft.com, and sign in with your work account.
   ...
   ![Sign in to https://flow.microsoft.com](media/flow-sign-in.png)
 
3. Upload the package.

    a. Click **My flows**.
 
   ![My flows page, highlighting the Import button](media/flow-my-import.png)
 
    b. Click **Import**.
 
   ![Import package page, highlighting the Upload button](media/flow-import-package.png)
 
    c. Click **Upload**, then select the **YammerFAQs.zip** file you downloaded.

   ![Import package page, showing the file being uploaded](media/flow-import-package-uploading.png)
 
4. After the package has been uploaded, modify the **IMPORT SETUP** configuration for the **Related resources**.
 
   ![Import package page, showing the Import Setup column](media/flow-create-as-new.png)
 
    a. In the QnA Maker resource, click **Select during import**.
 
   ![Import setup page, showing the Create new button](media/flow-setup-qna-maker.png)
 
    b. Click **+ Create new**.
 
   ![Flow page showing New Connection button](media/flow-new-connection.png)
 
    c. Click **+ New connection**, then search for **QnA Maker**.
 
   ![New connection page, showing search for QnA Maker](media/flow-new-connection-qna-maker.png)
 
    d. Click the **QnA Maker** provided by Microsoft.
 
   ![QnA Maker connection, showing the API Key box](media/flow-new-connection-qna-maker-api-key.png)
   ...
   ![Import setup page, showing the Refresh list button](media/flow-setup-qna-maker-2.png)
...
5. Use the steps under step 4 above as a guide to configure the rest of the connections (shown in the red rectangles in the picture below). Refer to the screenshots below for all the connections to help you search for and locate the appropriate connections.
   ...
   ![Related resources page, showing the Import Setup column](media/flow-setup-others.png)
 
    - **Yammer Connection**
 
   ![Yammer connector](media/flow-new-yammer-connection.png)
 
    - **Approvals Connection**
 
   ![Approval connector](media/flow-new-approval-connection.png)
 
    - **SharePoint Connection**
 
   ![SharePoint connector](media/flow-new-sharepoint-connection.png)
 
6. At the bottom of the page, click **Import**.
 
   ![Import page, showing importing in progress](media/flow-importing.png)
 
    a. After the Flow has successfully imported, click **Open flow**.
 
   ![Flow page, showing Open flow link](media/flow-imported.png)
 
7. Update the Flow.
 
   ![Flow page showing steps of flow](media/flow-yammer-faqs.png)
 
    a. Click the first node, and choose the Yammer group you created or used earlier.

   ![First page of flow, showing the Group Id and Network Id fields](media/flow-choose-yammer-group.png)
   ...
   ![Initialize variable for the Knowledge Base ID](media/flow-intialize-variable.png)
   ...
   ![Initialize variable for Approval Assigned To](media/flow-approval-assigned-to-variable.png)
   ...
   ![Create item, showing the Site Address field](media/flow-sp-create-item.png)
 
    f. Click **Save**.

   ![Flow, showing the Save button](media/flow-save.png)
...
2. To import this Flow, use the same process you used to import the previous Flow.
   ...
   ![Import package to Flow](media/flow-2-create-as-new.png)
...
3. Update the Flow.
   ...
   ![SharePoint Connector showing the site address](media/flow-when-new-item-created.png)
 
    b. Use the values you collected earlier to initialize **Knowledge Base Id** and **Cognitive Services Key**.
 
   ![Initialize Knowledge Base Id and Cognitive Services ID](media/flow-2-variables.png)
...
**Who is the target audience for the QnA Maker tool?**
   ...
   ![QnA Maker screen showing a question asked in the Yammer group](media/qna-maker-1.png)
 
Wait up to two minutes for an answer.
 
   ![Yammer conversation showing the answer to the previous question](media/yammer-qa-1.png)
...
**Please let me know is the QnA Maker cost-free?**
   ...
   ![QnA Maker screen showing a different question](media/qna-maker-2.png)
   ...
   ![Yammer conversation showing that QnA Maker needs to get more information](media/yammer-qa-2.png)
 
    b. The approver will then review the answer that the Cognitive Service wants to provide and then Approve it Outlook.
 
   ![Approval email with suggested answer pending approval](media/outlook-approve.png)
 
    c. A new answer will then be provided in the Yammer conversation
 
   ![Yammer conversation showing the answer to the previous question](media/yammer-qa-2-2.png)
...
**Can we create a live event in a Yammer group?**
   ...
   ![Yammer conversation showing response when no answer is found in knowledge base](media/yammer-qa-3.png)
   ...
   ![Approval email with suggested answer pending approval](media/outlook-reject.png)
 
    b. A new answer will be provided in the Yammer conversation
 
   ![Yammer conversation showing new answer](media/yammer-qa-3-2.png)
 
    c. The question asked will be added to the QnA list by the Yammer FAQs Flow.
 
   ![QnA list showing the new question](media/sp-qna-item-added.png)
 
    d. Finally, the newly added SharePoint list item will be added to the knowledge base by the Add Item to QnA Marker Flow.
 
   ![Knowledge base showing how new question is added](media/qna-maker-item-added.png)
